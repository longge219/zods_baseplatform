package com.zods.plugins.db.curd.service;
...
import com.zods.plugins.db.annotation.*;
import com.zods.plugins.db.bean.HashKeyValue;
import com.zods.plugins.db.cache.CacheHelper;
import com.zods.plugins.db.constant.BaseOperationEnum;
import com.zods.plugins.db.curd.entity.BaseEntity;
import com.zods.plugins.db.curd.entity.ZodsBaseEntity;
import com.zods.plugins.db.curd.mapper.ZodsBaseMapper;
import com.zods.plugins.db.curd.params.PageParam;
import com.zods.plugins.db.exception.BusinessException;
import com.zods.plugins.db.exception.BusinessExceptionBuilder;
import com.zods.plugins.db.utils.ApplicationContextUtils;
import com.zods.plugins.db.utils.ZodsAssert;
 ...
 * @date 2022-06-14
 **/
public interface ZodsBaseService<P extends PageParam, T extends BaseEntity> {

    ZodsBaseMapper<T> getMapper();
    ...
    default T selectOne(Long id) {
        ...
        ZodsAssert.notNull(t, "500-0001", new Object[0]);
    ...
    default IPage<T> page(P pageParam, Wrapper<T> wrapper) {
        ...
        if (StringUtils.isNotBlank(sort) && StringUtils.isNotBlank(order)) {
            if (sort.contains(",") && order.contains(",")) {
                ...
                if (orderSplit.length == sortSplit.length) {
                    ...
                    for(int i = 0; i < sortSplit.length; ++i) {
                        orderItems[i] = this.build(ZodsUtils.camelToUnderline(sortSplit[i]), orderSplit[i]);
                    }
                    ...
                    for(int i = 0; i < sortSplit.length; ++i) {
                        orderItems[i] = this.build(ZodsUtils.camelToUnderline(sortSplit[i]), order0);
                    }
                    page.addOrder(orderItems);
                }
            }
            if (sort.contains(",") && !order.contains(",")) {
                List<String> orderList = (List)Arrays.stream(sort.split(",")).map(ZodsUtils::camelToUnderline).collect(Collectors.toList());
            ...
            if (!sort.contains(",") && !order.contains(",")) {
                page.addOrder(new OrderItem[]{this.build(ZodsUtils.camelToUnderline(sort), order)});
            }
        }
        /**查询条件*/
        if (wrapper != null) {
    ...
    default Integer update(T entity) throws BusinessException {
        ...
        if (entity instanceof ZodsBaseEntity) {
            ZodsBaseEntity zodsBaseEntity = (ZodsBaseEntity)entity;
            T dbEntity = this.getById(zodsBaseEntity.getId());
        ...
        Arrays.stream(fields).filter((field) -> {
            ...
            try {
                ...
                if (field.isAnnotationPresent(Query.class) && StringUtils.isNotBlank(((Query)field.getAnnotation(Query.class)).column())) {
                    ...
                    column = ZodsUtils.camelToUnderline(field.getName());
                }
                boolean flag;
    ...
    default String formatKey(String key, String[] replaceArray, T entity) {
        if (key.contains("${")) {
            ...
            key = ZodsUtils.replaceFormatString(key, keyPatternMap);
            if (key.contains("${")) {
                return null;
            }
        }
    ...
    default String getColumn(SFunction<T, ?> function) {
        ...
        return ZodsUtils.camelToUnderline(fieldName);
    }
